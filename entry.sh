...
if [ $# == 0  ] || [ "${1#-}" != "$1" ]             ##### in case of no argument or if first argment is an option
   then                                             ##### (begins with "-") then start tinyproxy
      ...
      if [ -f /logs/tinyproxy.filter ]
         then
            optionsgrep="^Filter "
            echo "Filter \"/logs/tinyproxy.filter\""      > /tmp/tinyproxy.add
         else
            :>/tmp/tinyproxy.add
            ...
            case "$1"
               in
                  -[Uu]pstream)
                        if [ $# -gt 2 ] && [ "$3" = "${3#-}" ]
                           then
                              echo "Upstream http $2 \"$3\"" >> /tmp/tinyproxy.add
                              shift 3         || exitus 3 "wrong usage of '$1' -- see tinyproxy man page"
                           else
                              echo "Upstream http $2"        >> /tmp/tinyproxy.add
